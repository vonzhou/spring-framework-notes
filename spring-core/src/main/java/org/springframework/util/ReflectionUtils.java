...
public abstract class ReflectionUtils {
	...
	/**
	 * 在给定类（及其超类，或者超接口）匹配的方法上执行回调操作
	 * <p>
	 * The same named method occurring on subclass and superclass will appear twice,
	 * unless excluded by the specified {@link MethodFilter}.
	 * 
	 * @param clazz the class to introspect(内省)
	 * @param mc 在每个方法上要执行的回调
	 * @param mf 方法过滤器
	 */
				...
				for (Method existingMethod : methods) {
					if (method.getName().equals(existingMethod.getName())
						...
						if (existingMethod.getReturnType() != method.getReturnType()
								&& existingMethod.getReturnType().isAssignableFrom(
	 ...
	 * order to avoid the JVM's SecurityManager check and defensive array copying. In
	...
	/**
	 * Pre-built MethodFilter that matches all non-bridge methods which are not declared

...
class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			for (BeanFactoryPostProcessor postProcessor : beanFactoryPostProcessors) {
				if (postProcessor instanceof BeanDefinitionRegistryPostProcessor) {
					...
					// BeanDefinitionRegistryPostProcessor 在 BeanFactoryPostProcessor
					// 基础上有自己特殊处理，需要先调用
		...
		// Do not initialize FactoryBeans here: We need to leave all regular beans
		// uninitialized to let the bean factory post-processors apply to them!
		// 查找 BeanFactoryPostProcessor，前提也要是个 bean
		String[] postProcessorNames = beanFactory.getBeanNamesForType(
	...
	public static void registerBeanPostProcessors(
		...
		// 第一步，注册所有实现了 PriorityOrdered 的 BeanPostProcessors
		sortPostProcessors(beanFactory, priorityOrderedPostProcessors);
		registerBeanPostProcessors(beanFactory, priorityOrderedPostProcessors);

		// 第二步，注册所有实现了 Ordered 的 BeanPostProcessors
		...
		// 现在, 注册所有普通的 BeanPostProcessors（无序的）
		...
		// ApplicationListener 探测器
		beanFactory.addBeanPostProcessor(
				new ApplicationListenerDetector(applicationContext));
	}

	private static void sortPostProcessors(ConfigurableListableBeanFactory beanFactory,
			List<?> postProcessors) {

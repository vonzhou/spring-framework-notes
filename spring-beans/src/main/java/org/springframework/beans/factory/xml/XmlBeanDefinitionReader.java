...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public void setDocumentReaderClass(Class<?> documentReaderClass) {
		if (documentReaderClass == null
				|| !BeanDefinitionDocumentReader.class.isAssignableFrom(
						documentReaderClass)) {
	...
	public int loadBeanDefinitions(InputSource inputSource)
			throws BeanDefinitionStoreException {
		return loadBeanDefinitions(inputSource,
	...
	public int loadBeanDefinitions(InputSource inputSource, String resourceDescription)
		...
		return doLoadBeanDefinitions(inputSource,
				new DescriptiveResource(resourceDescription));
	}

	/**
	 * 从指定的 XML 文件中实际加载 bean definitions。这里先是 SAX 解析 XML 得到 Document 对象，然后根据 Document 注册
	 * BeanDefinition
	 * 
	 * @param inputSource the SAX InputSource to read from
	 * @param resource the resource descriptor for the XML file
	...
	/**
	 * 获取资源的验证模式 (DTD, XSD)：如果指定了验证模式（可以调用
	...
	protected int detectValidationMode(Resource resource) {
		if (resource.isOpen()) {
			throw new BeanDefinitionStoreException("Passed-in Resource [" + resource
		...
		try {
			// 比较独立的功能封装到一 Class 中，而不是直接一个方法调用！

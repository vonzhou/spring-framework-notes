 ...
 * autowires annotated fields, setter methods and arbitrary config methods. Such members
 * to be injected are detected through a Java 5 annotation: by default, Spring's
 ...
 *
 * <p>
 * Only one constructor (at max) of any given bean class may carry this annotation with
 * the 'required' parameter set to {@code true}, indicating <i>the</i> constructor to
 * autowire when used as a Spring bean. If multiple <i>non-required</i> constructors carry
 * the annotation, they will be considered as candidates for autowiring. The constructor
 * with the greatest number of dependencies that can be satisfied by matching beans in the
 * Spring container will be chosen. If none of the candidates can be satisfied, then a
 * default constructor (if present) will be used. An annotated constructor does not have
 * to be public.
 *
 * <p>
...
public class AutowiredAnnotationBeanPostProcessor
	...
	private final Set<String> lookupMethodsChecked = Collections.newSetFromMap(
	...
	/**
	 * Set the name of a parameter of the annotation that specifies whether it is
	 * required.
	 * 
	...
	public Constructor<?>[] determineCandidateConstructors(Class<?> beanClass,
		...
		if (!this.lookupMethodsChecked.contains(beanName)) {
			try {
							...
							public void doWith(Method method)
								...
								if (lookup != null) {
									LookupOverride override = new LookupOverride(method,
											lookup.value());
									try {
										RootBeanDefinition mbd = (RootBeanDefinition) beanFactory.getMergedBeanDefinition(
												beanName);
				...
				throw new BeanCreationException(beanName,
						"Lookup method resolution failed", ex);
			}
			catch (NoClassDefFoundError err) {
				throw new BeanCreationException(beanName,
						"Failed to introspect bean class [" + beanClass.getName()
								+ "] for lookup method metadata: could not find class that it depends on",
						err);
			}
		...
		Constructor<?>[] candidateConstructors = this.candidateConstructorsCache.get(
				beanClass);
				...
				if (candidateConstructors == null) {
					...
					for (Constructor<?> candidate : rawCandidates) {
						...
						if (ann == null) {
							...
							if (userClass != beanClass) {
								try {
									Constructor<?> superCtor = userClass.getDeclaredConstructor(
					...
					if (!candidates.isEmpty()) {
						...
						candidateConstructors = candidates.toArray(
								new Constructor<?>[candidates.size()]);
					}
					else if (rawCandidates.length == 1
							&& rawCandidates[0].getParameterTypes().length > 0) {
	...
	public void processInjection(Object bean) throws BeanCreationException {
		...
		try {
			...
			throw new BeanCreationException(
					"Injection of autowired dependencies failed for class [" + clazz
							+ "]",
					ex);
		}
	}
				...
				if (InjectionMetadata.needsRefresh(metadata, clazz)) {
					...
					try {
						...
						throw new IllegalStateException(
								"Failed to introspect bean class [" + clazz.getName()
										+ "] for autowiring metadata: could not find class that it depends on",
								err);
					}
				}
			}
			...
			ReflectionUtils.doWithLocalMethods(targetClass,
					new ReflectionUtils.MethodCallback() {
						...
						public void doWith(Method method)
							...
							AnnotationAttributes ann = findAutowiredAnnotation(
									bridgedMethod);
							if (ann != null && method.equals(
									ClassUtils.getMostSpecificMethod(method, clazz))) {
	...
	protected boolean determineRequiredStatus(AnnotationAttributes ann) {
		return (!ann.containsKey(this.requiredParameterName)
				|| this.requiredParameterValue == ann.getBoolean(
						...
						if (value != null || this.required) {
							...
							if (autowiredBeanNames.size() == 1) {
								...
								if (beanFactory.containsBean(autowiredBeanName)) {
									if (beanFactory.isTypeMatch(autowiredBeanName,
											field.getType())) {
	...
	private class AutowiredMethodElement extends InjectionMetadata.InjectedElement {
		...
		private Object[] resolveCachedArguments(String beanName) {
			...
			for (int i = 0; i < arguments.length; i++) {
				arguments[i] = resolvedCachedArgument(beanName,

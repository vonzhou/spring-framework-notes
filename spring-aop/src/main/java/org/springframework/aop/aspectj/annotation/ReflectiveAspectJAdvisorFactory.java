 ...
 * the AspectJ 5 annotation syntax, using reflection to invoke the corresponding advice
 * methods.
 *
 * @author Rod Johnson
...
public class ReflectiveAspectJAdvisorFactory extends AbstractAspectJAdvisorFactory
	...
	static {
		...
		comparator.addComparator(new ConvertingComparator<Method, Annotation>(
				new InstanceComparator<Annotation>(Around.class, Before.class,
						After.class, AfterReturning.class, AfterThrowing.class),
				new Converter<Method, Annotation>() {

					@Override
					public Annotation convert(Method method) {
						AspectJAnnotation<?> annotation = AbstractAspectJAdvisorFactory.findAspectJAnnotationOnMethod(
								method);
						return (annotation != null ? annotation.getAnnotation() : null);
					}
				}));
		comparator.addComparator(
				new ConvertingComparator<Method, String>(new Converter<Method, String>() {
	...
	public List<Advisor> getAdvisors(
		...
		if (!advisors.isEmpty()
			...
			// 如果该 Aspect 需要延迟初始化，那么在开头加入一个初始化的 Advisor
	...
	private List<Method> getAdvisorMethods(Class<?> aspectClass) {
		...
		ReflectionUtils.doWithMethods(aspectClass, new ReflectionUtils.MethodCallback() {
			...
			public void doWith(Method method) throws IllegalArgumentException {
				// Exclude pointcuts. 排除有 Pointcut 注解的方法
	...
	private Advisor getDeclareParentsAdvisor(Field introductionField) {
		...
		return new DeclareParentsAdvisor(introductionField.getType(),
				declareParents.value(), declareParents.defaultImpl());
	}
	...
	public Advice getAdvice(Method candidateAdviceMethod,
		...
		switch (aspectJAnnotation.getAnnotationType()) {
			case AtBefore:
				springAdvice = new AspectJMethodBeforeAdvice(candidateAdviceMethod,
						expressionPointcut, aspectInstanceFactory);
				break;
			case AtAfter:
				springAdvice = new AspectJAfterAdvice(candidateAdviceMethod,
						expressionPointcut, aspectInstanceFactory);
				break;
			...
			case AtAfterThrowing:
				...
				break;
			case AtAround:
				springAdvice = new AspectJAroundAdvice(candidateAdviceMethod,
						expressionPointcut, aspectInstanceFactory);
				break;
			case AtPointcut:
				if (logger.isDebugEnabled()) {
	...
	protected static class SyntheticInstantiationAdvisor extends DefaultPointcutAdvisor {
		...
		public SyntheticInstantiationAdvisor(
				final MetadataAwareAspectInstanceFactory aif) {
